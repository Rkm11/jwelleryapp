...
class Blog_Model extends CI_Model {
    ...
    public function getAllBlog($fields = '', $condition_to_pass = '', $order_by_to_pass = '', $limit_to_pass = '', $offset = '', $debug_to_pass = 0) {
        if ($fields == '')
            $fields = "b.*,(select category_name from " . $this->db->dbprefix('mst_blog_categories') . " bc where bc.category_id=b.category_id) as category_name,(select `url` from " . $this->db->dbprefix('mst_uri_map') . " u where u.rel_id=b.post_id and u.`type`='blog-post') as page_url";
    ...
    public function getRecentBlogs() {
        $this->db->select('b.post_short_description,u.url,b.blog_image');
        $this->db->from("mst_blog_posts as b");
        $this->db->join("mst_uri_map as u", 'u.rel_id = b.post_id','inner');
