                            ...
                            <table class="table table-bordered table-striped dataTable" id="example1" aria-describedby="example1_info">
                                ...
                                <tbody>
                                    ...
                                    foreach ($arr_contact_us as $contact) {
                                        ...
                                        <tr>
                                            <td ><center>
                                        <input value="<?php echo $contact['contact_id']; ?>" class="chkselect" type="checkbox">
                                    </center></td>
                                ...
                                <tfoot>
                                    <?php if ($cnt > 0) { ?>
                                    <th colspan="6"><input type="button" id="btn_delete_all" class="btn btn-danger" value="Delete Selected"></th>
                                    <?php } ?>
                                </tfoot>
                            </table>
                        </div>
        ...
        <script>
            jQuery(function () {
                jQuery(".select_all_button_class").next().on("click", function () {
                    ...
                    {
                        jQuery(".chkselect").parent().removeAttr("class");
                        jQuery(".chkselect").parent().attr("class", "icheckbox_minimal checked");

                    }
                    else
                    {
                        jQuery(".chkselect").parent().removeAttr("class");
                        jQuery(".chkselect").parent().attr("class", "icheckbox_minimal");
                    }
                });
                jQuery(".chkselect").next().on("click", function () {

                    if (jQuery('.chkselect').hasClass('checked')) {
            ...
            function updateSelectAll()
            {
                ...
                {
                    jQuery("#chk_all").removeAttr("checked");
                }
                else
                {
                    jQuery("#chk_all").attr("checked", "checked");
                }
            ...
            jQuery("#btn_delete_all").bind("click", function () {
                ...
                jQuery(".chkselect").each(function (index, element) {
                    if (jQuery(element).parent().hasClass("checked"))
                    {
                        var_bool = true;
                    }
                });
                ...
                if (var_bool == true) {
                    if (confirm("Are you sure to delete these contact us?"))
                    {
                        var arr_contact_ids = [];
                        ...
                        jQuery(".chkselect").each(function (index, element) {
                            ...
                            if (jQuery(element).parent().hasClass("checked"))
                                arr_contact_ids.push(jQuery(element).val());

                        });

                        var obj_params = new Object();
                        obj_params.contact_ids = arr_contact_ids;

                        jQuery.post("<?php echo base_url(); ?>backend/contact-us", obj_params, function (msg) {
                            if (msg.error == "1")
                            {
                                alert(msg.error_message);
                            }
