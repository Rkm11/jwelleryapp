...
class User extends CI_Controller {
    ...
    public function listWUser() {
       ...
       /*using the user model*/
        $data['title'] = "Manage User";
        $data['arr_user_list'] = $this->user_model->getUserDetails('3');
        $this->load->view('backend/user/list', $data);
    }
...
//        $this->form_validation->set_rules('user_name', 'user username', 'required');
...
//                        "user_name" => mysql_real_escape_string($this->input->post('user_name')),
                        ...
                        'role_id' => 1,
                        'user_password' => base64_encode($this->input->post('user_password')),
                        'about_me' => mysql_real_escape_string($this->input->post('about_me')),
                    );
                } else {
   ...
   /*function to display admin profile*/
    ...
    function reverificationLink($user_id = 0, $user_type) {
        ...
        if ($user_id != '' && $user_type != '') {
            ...
            $user_name = $data['arr_user_details'][0]['user_name'];
